...
data_cleaned = data[data['Monoisotopic Mass'].notnull()].reset_index(drop=True)
...
def mass_sum(data, full_mass, tolerance=0.1):
    ...
    results = []
    for i, mass1 in enumerate(data['Monoisotopic Mass']):
        for j, mass2 in enumerate(data['Monoisotopic Mass']):
...
def gap_fill(data, mass_pairs, tolerance=0.5):
    ...
    for _, row in mass_pairs.iterrows():
        for mass in data['Monoisotopic Mass']:
...
def plot_mass_distribution(data, title):
    ...
    plt.hist(data['Monoisotopic Mass'], bins=30, alpha=0.7, label='Mass Distribution')
